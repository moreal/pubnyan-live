...
- [!] **Smoke: loop plumbing.** Add the line `loop smoke 2026-09-06` to the end of `docs/backlog-smoke.txt` (create it). Done when: `tail -1 docs/backlog-smoke.txt` prints `loop smoke 2026-09-06` and `npm run check` is green.
  - failed 2026-09-06: loop plumbing.: work is complete and reviewed PASS (docs/backlog-smoke.txt added, `npm run check` green), but `git_commit` keeps refusing with `git add -A failed: ... index file smaller than expected` — the harness process has the pre-fix `src/tools/tree-hash.ts` cached in memory; the committed-in-tree fix is verified manually and a restarted loop should commit cleanly.

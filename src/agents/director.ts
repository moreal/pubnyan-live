...
export const artist = defineSubagent({
  name: 'artist',
  description:
    'Implements one backlog item with a graphics/visual component (new artwork, SVG structure, clip look-and-feel, Lottie/Rive asset shape) and reports what it changed and how it verified it.',
  agent: Artist,
...
2. Decide who implements it: if the item is primarily about graphics or visual result (new artwork, SVG structure, a clip's look-and-feel, a Lottie/Rive asset's shape), delegate to the artist; otherwise delegate to the implementer. Either way, pass the item's full text and section verbatim and say: implement it, prove it with run_checks, report back.
3. Delegate to the reviewer with the item's full text and the implementer's (or artist's) report. If it replies FINDINGS, re-delegate to whichever of the implementer/artist did the work and then re-review. Allow at most 3 review rounds in total; count them yourself.
4. When the reviewer replies PASS: call mark_done with the item's exact title, then call git_commit with a conventional message that names the item (for example "feat(motion): add wink clip (backlog: Smoke: wink clip)"). git_commit runs the check suite itself and refuses if it fails; if it refuses, the item stays checked in the working tree; that is fine, mark_done is idempotent. Re-delegate to whichever of the implementer/artist did the work with the refusal as findings, and after the next PASS call mark_done and git_commit again.

Every delegation spawns a FRESH child that remembers nothing of the earlier rounds, so each re-delegation in steps 3 and 4 must carry, in full: the item's text and section verbatim, the previous implementer's or artist's report, and the reviewer's findings (or git_commit's refusal and its check summary) verbatim. Bound each delegation prompt to what the child needs; do not paste tool transcripts.

...
test('cry transitions conceal incompatible eye crossfades behind a closed blink', () => {
  const rig = getRig('pubnyan');
  for (const clip of clips.filter(c => ['to-cry', 'from-cry'].includes(c.name))) {
    ...
    for (const part of rig.parts.filter(p => p.name.startsWith('eye-'))) {
      ...
      for (let frame = 1; frame < 96; frame++) {
        ...
        if (paths.length > 1) {
          ...
          if (part.name.endsWith('.pupil')) expect(sampleNumeric(channels.opacity!, time)).toBe(0);
        }
      }
    }
  }
